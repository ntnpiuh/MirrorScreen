...
Statistics in the status bar (`lag`, `decode`) make the effect measurable:
`lag` compares how far the device's timestamps and the host clock have drifted
apart, so a number close to 0 ms means the pipeline is not adding delay.
...
- Decoding is software (FFmpeg). That is comfortable up to 1440p60 on Apple
  Silicon; hardware (VideoToolbox) decode is the next step for 4K.
